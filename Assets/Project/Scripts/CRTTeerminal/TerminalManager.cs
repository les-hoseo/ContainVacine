...
{
    public TabState currentTab = TabState.ROOT;
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.LeftAlt) && !rootTerminalController.isTyping && !dialogTerminalController.isTyping)
        {
            ToggleTab();
        }
    }
    ...
    {
        var lines = commandManager.Process(input, currentTab);
        // 출력 처리
    }
}