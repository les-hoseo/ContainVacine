...
// 이 enum은 변경할 필요가 없습니다.

// --- ✨수정된 부분: 파일 이름과 메뉴 경로를 새 이름으로 변경 ---
[CreateAssetMenu(fileName = "StoryData_FEAR", menuName = "Scriptable Objects/StoryData_FEAR")]
public class StoryData_FEAR : ScriptableObject // ✨클래스 이름 변경
{
    // --- ✨수정된 부분: 리스트가 담을 데이터 타입을 Data_FEAR로 변경 ---
    public List<Data_FEAR> Story = new List<Data_FEAR>();
}

[System.Serializable]
public class Data_FEAR // ✨내부 데이터 클래스 이름도 일관성을 위해 변경
{
    public string Name;
    [TextArea(3, 5)]
    ...
    [Tooltip("FadeIn 효과를 줄 때 사용할 이미지")]
